...
const WORKSPACE_CONFIG: Record<string, { initials: string; color: string }> = {
  general:        { initials: 'AH', color: '#6366f1' },
  equifind:       { initials: 'EQ', color: '#f59e0b' },
  home_inspection:{ initials: 'HI', color: '#10b981' },
};

export function AppLayout({ children }: { children: React.ReactNode }) {
  const [location] = useLocation();
  const { logout, account, businessTag } = useAppStore();

  const wsId = account?.workspace ?? businessTag ?? 'general';
  const wsCfg = WORKSPACE_CONFIG[wsId] ?? WORKSPACE_CONFIG.general;
      ...
      <aside className="w-[68px] flex-shrink-0 bg-[#090b12] border-r border-white/5 flex flex-col items-center py-4 gap-2 z-20">
        ...
        <div className="relative group mb-2">
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${wsCfg.color}, ${wsCfg.color}88)` }}
          >
            {wsCfg.initials}
          </div>
          {/* Tooltip showing workspace name */}
